...
* 2 The Rec Format:  Nothing to comply with.
  ...
  * 2.4 Record Descriptors:  Nothing to comply with.
    * 2.4.1 Record Sets:  Recordset type example, Two record descriptors in the same database, TODO empty recordset example, TODO Default record type (untyped) mixed with typed record types. (TODO add checks for recordsets.len() == 0 for empty DB or with just comments etc. and for for recordsets.len() in general)
